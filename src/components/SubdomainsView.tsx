...
import { Network, Search, Copy, Check, ExternalLink, ShieldCheck, Globe, Database, Server } from 'lucide-react';
...
export const SubdomainsView: React.FC<Props> = ({ subdomains, rootDomain }) => {
      ...
      <div className="flex flex-wrap items-center justify-between border-b border-slate-800/80 pb-4 gap-4">
        <div className="space-y-1">
          ...
          <p className="text-xs text-slate-400 font-mono">
            Ecosystem hostnames identified via Certificate Transparency (crt.sh) and public web crawls.
          </p>
        </div>

        <div className="flex items-center gap-3">
          <span className="text-xs text-emerald-300 bg-emerald-500/10 border border-emerald-500/30 px-3 py-1 rounded-full font-mono font-bold flex items-center gap-1.5">
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3.5">
          {filtered.map((item, idx) => (
            <div
              key={idx}
              className="bg-slate-950/90 border border-slate-800/90 hover:border-emerald-500/40 rounded-xl p-4 space-y-3 transition-all group shadow-md"
            >
