...
export const SnapshotComparison: React.FC<Props> = ({ snapshots, domain }) => {
  ...
  const targetSnapshot =
    snapshots.find((s) => s.id === targetId) || snapshots[snapshots.length - 1];
      ...
      <div className="flex flex-wrap items-center justify-between border-b border-slate-800/80 pb-5 gap-4">
        <div className="space-y-1">
          ...
          <p className="text-xs text-slate-400 font-mono">
            Compare any two historical points in time to inspect framework migrations, payload
            deltas, and UX shifts.
          </p>
        </div>
          ...
          >
            {snapshots.map((s, idx) => (
              <option key={`${s.id}-${idx}`} value={s.id}>
                {s.timestamp.split('T')[0]} ({new Date(s.timestamp).getFullYear()}) —{' '}
                {(s.contentLength / 1024).toFixed(1)} KB — HTTP {s.statusCode}
              </option>
            ))}
          </select>
        </div>
        ...
        <div className="h-44 w-full pt-2">
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <Bar
                dataKey="baseline"
                name={`Baseline (${baseYear})`}
                fill="#f59e0b"
                radius={[0, 4, 4, 0]}
              />
              <Bar
                dataKey="target"
                name={`Target (${targetYear})`}
                fill="#10b981"
                radius={[0, 4, 4, 0]}
              />
            </BarChart>
        ...
        <div className="bg-slate-950/90 border border-slate-800 rounded-xl p-5 space-y-4 shadow-lg">
          ...
          <div className="grid grid-cols-2 gap-3 text-xs font-mono">
            <div className="bg-slate-900/80 p-3 rounded-lg border border-slate-800/80">
              ...
              <span className="font-bold text-slate-200">
                {baseSnapshot.timestamp.split('T')[0]}
              </span>
            </div>
            <div className="bg-slate-900/80 p-3 rounded-lg border border-slate-800/80">
              <span className="text-slate-500 uppercase block text-[10px]">Payload Weight</span>
              <span className="font-bold text-slate-200">
                {(baseSnapshot.contentLength / 1024).toFixed(1)} KB
              </span>
            </div>
          </div>
          ...
          <div className="space-y-2">
            <span className="text-[11px] font-mono font-bold text-slate-400 uppercase">
              Observed Tech Signatures ({baseSnapshot.detectedTech.length}):
            </span>
            <div className="flex flex-wrap gap-1.5">
              {baseSnapshot.detectedTech.map((t, idx) => (
                <span
                  key={`${t.id}-${idx}`}
                  className="px-2.5 py-1 bg-slate-900 border border-slate-800 text-slate-300 rounded-md text-xs font-mono flex items-center gap-1.5"
                >
        ...
        <div className="bg-slate-950/90 border border-slate-800 rounded-xl p-5 space-y-4 shadow-lg">
          ...
          <div className="grid grid-cols-2 gap-3 text-xs font-mono">
            <div className="bg-slate-900/80 p-3 rounded-lg border border-slate-800/80">
              <span className="text-slate-500 uppercase block text-[10px]">Crawl Timestamp</span>
              <span className="font-bold text-slate-200">
                {targetSnapshot.timestamp.split('T')[0]}
              </span>
            </div>
            <div className="bg-slate-900/80 p-3 rounded-lg border border-slate-800/80 flex items-center justify-between">
              <div>
                <span className="text-slate-500 uppercase block text-[10px]">Payload Weight</span>
                <span className="font-bold text-slate-200">
                  {(targetSnapshot.contentLength / 1024).toFixed(1)} KB
                </span>
              </div>
              <span
                className={`text-[10px] px-2 py-0.5 rounded font-bold flex items-center gap-0.5 ${
          ...
          <div className="space-y-2">
            <span className="text-[11px] font-mono font-bold text-slate-400 uppercase">
              Observed Tech Signatures ({targetSnapshot.detectedTech.length}):
            </span>
            <div className="flex flex-wrap gap-1.5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs font-mono">
          ...
          <div className="bg-emerald-950/20 border border-emerald-500/30 rounded-xl p-4 space-y-2">
            <div className="flex items-center justify-between text-emerald-400 font-bold">
              <span className="flex items-center gap-1.5">
                <PlusCircle className="w-4 h-4" /> Adopted Technologies
              </span>
              <span className="text-[10px] bg-emerald-500/20 px-2 py-0.5 rounded-full">
                {comparison.addedTech.length}
              </span>
            </div>
            {comparison.addedTech.length === 0 ? (
              ...
              <div className="space-y-1.5 pt-1">
                {comparison.addedTech.map((t, idx) => (
                  <div
                    key={`${t.id}-${idx}`}
                    className="bg-slate-900/90 p-2 rounded-lg border border-emerald-500/20 text-emerald-300 font-bold text-xs flex items-center justify-between"
                  >
                    <span>{t.name}</span>
          ...
          <div className="bg-red-950/20 border border-red-500/30 rounded-xl p-4 space-y-2">
            <div className="flex items-center justify-between text-red-400 font-bold">
              <span className="flex items-center gap-1.5">
                <MinusCircle className="w-4 h-4" /> Decommissioned
              </span>
              <span className="text-[10px] bg-red-500/20 px-2 py-0.5 rounded-full">
                {comparison.removedTech.length}
              </span>
            </div>
            {comparison.removedTech.length === 0 ? (
              ...
              <div className="space-y-1.5 pt-1">
                {comparison.removedTech.map((t, idx) => (
                  <div
                    key={`${t.id}-${idx}`}
                    className="bg-slate-900/90 p-2 rounded-lg border border-red-500/20 text-red-300 font-bold text-xs flex items-center justify-between"
                  >
                    <span>{t.name}</span>
                    <span className="text-[10px] text-slate-400">{t.category}</span>
                  </div>
          ...
          <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-4 space-y-2">
            <div className="flex items-center justify-between text-slate-300 font-bold">
              <span className="flex items-center gap-1.5">
                <CheckCircle className="w-4 h-4 text-blue-400" /> Retained Across Eras
              </span>
              <span className="text-[10px] bg-slate-800 px-2 py-0.5 rounded-full">
                {comparison.retainedTech.length}
              </span>
            </div>
            {comparison.retainedTech.length === 0 ? (
              ...
              <div className="space-y-1.5 pt-1">
                {comparison.retainedTech.map((t, idx) => (
                  <div
                    key={`${t.id}-${idx}`}
                    className="bg-slate-950/80 p-2 rounded-lg border border-slate-800 text-slate-300 text-xs flex items-center justify-between"
                  >
                    <span>{t.name}</span>
