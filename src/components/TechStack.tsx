      ...
      <div className="flex flex-wrap items-center justify-between border-b border-slate-800/80 pb-5 gap-4">
        <div className="space-y-1">
          ...
          <p className="text-xs text-slate-400 font-mono">
            Distinguishing direct protocol header observations from heuristic HTML DOM fingerprints
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {categories.map((cat) => {
            ...
            return (
              <div
                key={cat}
                className="bg-slate-950/80 border border-slate-800/80 rounded-xl p-5 space-y-4 hover:border-slate-700 transition-all"
              >
                ...
                <div className="space-y-3">
                  {catTechs.map((tech, techIdx) => (
                    <div
                      key={`${tech.id}-${techIdx}`}
                      className="bg-slate-900/90 border border-slate-800/80 rounded-xl p-3.5 space-y-2.5 hover:border-purple-500/40 transition-all"
                    >
