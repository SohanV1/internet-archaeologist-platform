...
export function computeStoryMilestones(
  ...
  if (snapshots.length > 0) {
    ...
    for (let i = 1; i < snapshots.length; i++) {
      ...
      const prevFrameworks = prev.detectedTech.filter(
        (t) => t.category === 'JavaScript Framework' || t.category === 'CMS'
      );
      const currFrameworks = curr.detectedTech.filter(
        (t) => t.category === 'JavaScript Framework' || t.category === 'CMS'
      );
  ...
  if (subdomains.length > 0) {
    ...
    milestones.push({
      ...
      impact: 'major',
      details: subdomains.map(
        (s) => `${s.fullDomain} (${s.source}${s.firstSeen ? ' - First seen ' + s.firstSeen : ''})`
      ),
    });
  }
...
export function generateExecutiveSummary(
  ...
  const narrative = `Target ${domain} first surfaced in public web archives in ${earliestYear} (active for ~${totalYears} years). Over its lifecycle, our passive reconnaissance identified ${milestones.length} major structural events, including ${migrations} framework migrations, ${redesigns} UI/UX redesign cycles, and an ecosystem spanning ${subdomains.length} subdomains. Today, the domain operates on ${activeTech
    .map((t) => t.name)
    .slice(0, 3)
    .join(', ')}.`;

  const hasSecurityHeader = activeTech.some(
    (t) => t.category === 'Security' || t.name.includes('HSTS')
  );
  const hasCDN = activeTech.some(
    (t) => t.category === 'CDN/Hosting' || t.name.toLowerCase().includes('cloudflare')
  );

  const securityRating: ExecutiveSummary['securityRating'] =
    hasSecurityHeader && hasCDN ? 'High' : hasSecurityHeader || hasCDN ? 'Moderate' : 'Basic';
