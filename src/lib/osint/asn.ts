...
const KNOWN_ASNS: Record<string, { asn: string; org: string; country: string; isp: string }> = {
  ...
  '93.184': {
    ...
    country: 'US',
    isp: 'Edgecast Anycast CDN',
  },
  '151.101': {
    asn: 'AS54113',
    org: 'Fastly, Inc.',
    country: 'US',
    isp: 'Fastly Global Edge Network',
  },
  '76.76': {
    asn: 'AS8987',
    org: 'Amazon.com, Inc. (Vercel)',
    country: 'US',
    isp: 'Vercel Edge Platform',
  },
...
export function lookupAsnInfo(ipAddresses: string[]): AsnInfo[] {
  ...
  return ipAddresses.map((ip) => {
    ...
    const cidr =
      segments.length === 4 ? `${segments[0]}.${segments[1]}.${segments[2]}.0/24` : `${ip}/64`;
