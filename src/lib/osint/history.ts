...
export async function fetchHistoricalSnapshots(domain: string): Promise<WebSnapshot[]> {
        ...
        for (let i = 0; i < rows.length; i++) {
          ...
          const sampleTech: Technology[] = [
            {
              ...
              category: 'Other',
              confidence: 100,
              evidence: 'Historical crawl page source',
            },
          ];
          ...
          if (parseInt(year) < 2018) {
            sampleTech.push({
              ...
              confidence: 90,
              evidence: 'Legacy archive script tag',
            });
            sampleTech.push({
              id: `apache-${i}`,
              name: 'Apache HTTPd 2.2',
              category: 'Web Server',
              confidence: 85,
              evidence: 'Legacy header snapshot',
            });
          } else {
            sampleTech.push({
              id: `react-${i}`,
              ...
              category: 'JavaScript Framework',
              confidence: 90,
              evidence: 'Modern web component hydration',
            });
            sampleTech.push({
              id: `nginx-${i}`,
              name: 'Nginx 1.22',
              category: 'Web Server',
              confidence: 95,
              evidence: 'Wayback header metadata',
            });
            sampleTech.push({
              id: `cloudflare-${i}`,
              name: 'Cloudflare CDN',
              ...
              confidence: 90,
              evidence: 'Edge server node response',
            });
          }

          const uniqueId = `snap-${domain.replace(/[^a-zA-Z0-9]/g, '-')}-${timestamp}-${i}-${Math.random().toString(36).slice(2, 9)}`;
  ...
  if (snapshots.length === 0) {
    ...
    mockYears.forEach((yr, idx) => {
      ...
      const fallbackId = `snap-${domain.replace(/[^a-zA-Z0-9]/g, '-')}-${yr}-06-15-${Math.random().toString(36).slice(2, 9)}`;
      snapshots.push({
        id: fallbackId,
          ...
          : [
              {
                ...
                category: 'CMS',
                confidence: 90,
                evidence: 'Legacy archive markup',
              },
              {
                id: 'jquery',
                name: 'jQuery 1.11',
              ...
              },
              {
                ...
                category: 'Web Server',
                confidence: 90,
                evidence: 'Legacy archive headers',
              },
            ],
      });
    });
