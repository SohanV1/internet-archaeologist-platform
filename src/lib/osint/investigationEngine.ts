...
export async function createInvestigation(domainInput: string): Promise<Investigation> {
  ...
  let htmlSample =
    '<!DOCTYPE html><html><head><title>Observed Site</title></head><body><div id="__next"></div></body></html>';
  ...
  try {
    const res = await fetchWithRetry(
      ...
      {
        method: 'GET',
        headers: { 'User-Agent': 'Internet-Archaeologist-OSINT/1.5' },
      },
  ...
  const milestones = computeStoryMilestones(
    linkedSnapshots,
    linkedSubdomains,
    detectedTech,
    domain
  ...
  // Compute real SHA-256 forensic hashes using the Web Crypto API
  const [dnsHash, subdomainsHash, httpHash, waybackHash, certsHash, asnHash] = await Promise.all([
    calculateSha256(JSON.stringify(linkedDnsRecords)),
    calculateSha256(JSON.stringify(linkedSubdomains)),
    calculateSha256(rawResponseHeaders),
    calculateSha256(JSON.stringify(linkedSnapshots.map((s) => s.id))),
    calculateSha256(JSON.stringify(linkedCertificates)),
    calculateSha256(JSON.stringify(linkedAsnInfo)),
  ]);
  ...
  const evidence: EvidenceItem[] = [
    ...
    },
    {
      ...
      confidenceScore: 95,
      collectionMethod:
        'Passive Certificate Transparency log mining and historical CDX wildcard index search',
      relatedEntity: domain,
      relatedObservation: `Discovered ${linkedSubdomains.length} subdomains associated with root zone ${domain}`,
      observationNature: 'OBSERVED',
      verificationHash: subdomainsHash,
    },
    {
      id: httpEvidenceId,
      timestamp: now,
    ...
    },
    {
      ...
      notes:
        'Mapped resolved IP addresses to Autonomous System Numbers (ASN), BGP prefixes, and cloud providers',
      confidence: 'HIGH',
      confidenceScore: 95,
  ...
  for (const tech of detectedTech) {
    ...
    const techHash = await calculateSha256(`${tech.name}:${tech.evidence}`);
    evidence.push({
      id: techEvId,
      timestamp: now,
      source:
        tech.observationNature === 'OBSERVED'
          ? `HTTP Response Headers (${targetUrl})`
          : `HTML Source Analysis (${targetUrl})`,
      sourceUrl: targetUrl,
      ...
      confidence:
        ...
        (tech.confidence >= 90 ? 'HIGH' : tech.confidence >= 70 ? 'MEDIUM' : 'LOW'),
      confidenceScore: tech.confidence,
      collectionMethod:
        tech.observationNature === 'OBSERVED'
          ? 'HTTP response header inspection'
          : 'Static HTML regex fingerprint signature matching',
